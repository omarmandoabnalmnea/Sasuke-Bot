...
*Nota:* Este comando solo podrá ser usado por owners del Bot.`.trim();
  ...
  conn.sendMessage(m.chat, {text: `🗂️ الخيار: ${type}\n🎚️ حالة: ${isEnable ? 'نشط' : 'غير نشط'}\n📣 الفقرة: ${isAll ? 'عام' : isUser ? '' : 'للجروب'}`}, {quoted: m});
};
handler.help = ['on', 'off'].map((v) => v + 'able <option>');
handler.tags = ['group', 'owner'];
handler.command = /^((on|off)|(نيتيت|تتسنسنن)ل|(turn)?[01])$/i;

...
if (!img) throw '*[❌] لقد نسيت ارسال صورة أو رد عليها*'
await conn.updateProfilePicture(m.chat, img).then(_ => m.reply('*تم تغير الصورة بنجاح ✅*'))
} else throw '*[❌] لقد نسيت ارسال صورة أو رد عليها*'}
handler.command = /^تغير-الصوره?|تغير-الصورة$/i
handler.group = true
handler.admin = true
